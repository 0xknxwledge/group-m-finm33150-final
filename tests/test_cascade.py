...
class TestIsLiquidated:
    def test_safe_position(self):
        pos = Position(collateral_usd=200, debt_usd=100, liquidation_threshold=1.25, layer="perp")
        assert _is_liquidated(pos, 0.05) is False

    def test_underwater_position(self):
        pos = Position(collateral_usd=120, debt_usd=100, liquidation_threshold=1.0, layer="perp")
    ...
    def test_exact_threshold_not_liquidated(self):
        ...
        pos = Position(collateral_usd=100, debt_usd=100, liquidation_threshold=1.0, layer="perp")
        assert _is_liquidated(pos, 0.0) is False

    def test_zero_debt_never_liquidated(self):
        pos = Position(collateral_usd=1000, debt_usd=0, liquidation_threshold=1.25, layer="hyperlend")
        assert _is_liquidated(pos, 0.99) is False

    def test_full_shock_liquidates(self):
        pos = Position(collateral_usd=200, debt_usd=100, liquidation_threshold=1.25, layer="morpho")
...
class TestSimulateCascade:
    ...
    def _safe_positions() -> list[Position]:
        return [
            Position(collateral_usd=10_000, debt_usd=1_000, liquidation_threshold=1.25, layer="perp"),
            Position(collateral_usd=10_000, debt_usd=1_000, liquidation_threshold=1.25, layer="hyperlend"),
        ]
    ...
    def _fragile_positions() -> list[Position]:
        ...
        return [
            ...
            Position(collateral_usd=1020, debt_usd=1000, liquidation_threshold=1.0, layer="perp"),
            # Survives 5% but not the additional price impact from round 1
            Position(collateral_usd=1100, debt_usd=1000, liquidation_threshold=1.0, layer="hyperlend"),
            # Even more cushion — needs round 2 impact
            Position(collateral_usd=1200, debt_usd=1000, liquidation_threshold=1.0, layer="morpho"),
        ]
    ...
    def test_multi_round_cascade(self):
        result = simulate_cascade(
            self._fragile_positions(), 100.0, 0.05, orderbook_depth_usd=500_000,
        )
    ...
    def test_layer_tracking(self):
        result = simulate_cascade(
            self._fragile_positions(), 100.0, 0.10, orderbook_depth_usd=500_000,
        )
    ...
    def test_max_rounds_cap(self):
        ...
        huge = [
            Position(collateral_usd=1e9, debt_usd=1e9, liquidation_threshold=1.0, layer="perp")
            for _ in range(200)
        ]
        result = simulate_cascade(huge, 100.0, 0.01, orderbook_depth_usd=1.0, max_rounds=5)
...
class TestAmplificationCurve:
    ...
    def _positions() -> list[Position]:
        return [
            Position(collateral_usd=1100, debt_usd=1000, liquidation_threshold=1.0, layer="perp"),
        ]
...
class TestCascadeRiskSignal:
    def test_low_risk(self):
        safe = [
            Position(collateral_usd=10_000, debt_usd=100, liquidation_threshold=1.25, layer="perp"),
        ]
