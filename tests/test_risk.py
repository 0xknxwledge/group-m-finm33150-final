...
def _safe_positions() -> list[Position]:
    return [
        Position(collateral_usd=500, debt_usd=500, liquidation_threshold=0.005, layer="perp"),
    ]


def _fragile_positions() -> list[Position]:
    return [
        Position(collateral_usd=20, debt_usd=980, liquidation_threshold=0.005, layer="perp")
        for _ in range(20)
    ]
...
class TestJumpWeightedRisk:
    def test_output_keys(self):
        ...
        expected_keys = {
            "baseline_loss", "amplified_loss", "cascade_excess",
    ...
    def test_fragile_higher_amplification(self):
        safe = jump_weighted_risk(_make_params(), _safe_positions(),
                                  orderbook_depth_usd=100_000, n_shocks=20)
        fragile = jump_weighted_risk(_make_params(), _fragile_positions(),
                                     orderbook_depth_usd=100_000, n_shocks=20)
...
class TestJumpWeightedRiskAllCoins:
    ...
    def test_depth_per_coin(self):
        ...
        results = jump_weighted_risk_all_coins(
            params_dict, oi_df, depth_per_coin=depth,
        )
        assert "BTC" in results
